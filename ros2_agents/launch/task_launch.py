...
def resolve_model_description(model_path, pkg_dir, name, color, lwh):
  if model_path.endswith(".urdf.xacro"):
    ...
    raise ValueError("Model path must end with .urdf.xacro or .sdf")
...
def generate_launch_description() -> LaunchDescription:
  ...
  set_gz_resource_path = SetEnvironmentVariable(name='IGN_FILE_PATH',
                                                value=[
                                                    EnvironmentVariable('IGN_FILE_PATH',
                                                                        default_value=''), ':',
                                                    path.join(ros2_agents_dir, 'sim/third_party')
                                                ])
  set_gz_sdf_path = SetEnvironmentVariable(name='SDF_PATH',
                                           value=[
  ...
  bridge = Node(
      ...
      parameters=[
          {
              "config_file": path.join(ros2_agents_dir, "config/ros_gz_bridge.yaml")
          },
          # {
          #     'qos_overrides': {
          #         '/rgbd_camera/points': {
  ...
  demo_app_node = Node(
      ...
      ros_arguments=["--log-level", LaunchConfiguration("log-level")],
  )
...
#           rviz_config_file='src/ros2_transformers/ros2_agents/config/rviz_gui.rviz',
#           use_sim_time='true').items())
  ...
  return LaunchDescription([
      ...
      *spawn_item_nodes,
      ...
      demo_app_node,
    #   RegisterEventHandler(
    #       OnProcessStart(target_action=demo_app_node,
    #                      on_start=[LogInfo(msg='App started, launching moveit'), TimerAction(period=0.5, actions=[
    #                    moveit_launch])])),
  ])
